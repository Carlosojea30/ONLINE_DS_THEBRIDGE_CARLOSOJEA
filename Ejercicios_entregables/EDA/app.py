...
# -----------------------------
st.set_page_config(
    page_title="Pokédex Competitiva",
    page_icon="🎮",
    layout="wide"
)
...
inject_bg_image("img/pokeball_bg.png", opacity=0.09, size="900px")

# -----------------------------
# Carga de datos
# -----------------------------
@st.cache_data
def load_data(path="data/pokemon_unified_enriched.csv"):
    df = pd.read_csv(path)
    for c in ["name", "type1", "type2"]:
        if c in df.columns:
            df[c] = df[c].astype(str).str.strip().str.lower()
    need = ["id","name","type1","type2","hp","attack","defense","sp_attack",
            "sp_defense","speed","base_total","height","weight","abilities",
            "sprite_png","sprite_svg","generation","legendary",
...
def infer_roles(df):
    ...
    def _rol(r):
        atk_hi   = (r["attack"] >= pct["attack"]["hi"]) or (r["sp_attack"] >= pct["sp_attack"]["hi"])
        ...
        def_mid  =  (r["defense"] >= pct["defense"]["mid"]) or (r["sp_defense"] >= pct["sp_defense"]["mid"])
...
def type_advantage(a_row, b_row):
    ...
    for t in a_types:
        if t in b_weak:
            score_a += 2; reasons_a.append(f"{t.capitalize()} es supereficaz vs {', '.join(b_types)}")
        if t in b_resist:
            score_a -= 1; reasons_a.append(f"{t.capitalize()} es poco eficaz vs {', '.join(b_types)}")
        if t in b_immune:
            score_a -= 3; reasons_a.append(f"{t.capitalize()} no afecta a {', '.join(b_types)}")

    for t in b_types:
        if t in a_weak:
            score_b += 2; reasons_b.append(f"{t.capitalize()} es supereficaz vs {', '.join(a_types)}")
        if t in a_resist:
            score_b -= 1; reasons_b.append(f"{t.capitalize()} es poco eficaz vs {', '.join(a_types)}")
        if t in a_immune:
            score_b -= 3; reasons_b.append(f"{t.capitalize()} no afecta a {', '.join(a_types)}")

    return (score_a, reasons_a), (score_b, reasons_b)

def decide_duel(row_a, row_b):
    score_a = float(row_a.get("base_total", 0))/100.0 + float(row_a.get("speed", 0))/50.0
    score_b = float(row_b.get("base_total", 0))/100.0 + float(row_b.get("speed", 0))/50.0
    (ta, ra), (tb, rb) = type_advantage(row_a, row_b)
    score_a += ta; score_b += tb
...
with st.sidebar:
    ...
    menu = st.radio(
        ...
        ["🏠 Portada", "🧭 Idea de negocio", "📄 Ficha Pokémon", "⚔️ Comparador", "📊 Análisis competitivo", "📐 Conclusión estadística"],
        index=0
    )
    st.divider()
    st.caption("🎵 Música (haz click en ▶️ para reproducir)")
    st.audio("music/pokemon_theme.mp3")
...
# -----------------------------
    ...
    stats = row[["hp","attack","defense","sp_attack","sp_defense","speed"]].astype(float)
    st.bar_chart(stats)

    st.markdown("### Tipos: debilidades / resistencias / inmunidades")
    colw, colr, coli = st.columns(3)
    with colw: st.write("**Débil a:**", row.get("weak_to","-"))
    with colr: st.write("**Resiste:**", row.get("resist_to","-"))
    with coli: st.write("**Inmune a:**", row.get("immune_to","-"))
...
# -----------------------------
    ...
    with colA:
        poke_a = st.selectbox("Pokémon A", sorted(df["name"].dropna().unique()), index=0)
    ...
    with colB:
        poke_b = st.selectbox("Pokémon B", sorted(df["name"].dropna().unique()), index=1)
    ...
    if winner == "A":
        st.success(f"🏆 **{row_a['name'].capitalize()}** gana el duelo (margen {margin:.2f}).")
    elif winner == "B":
        st.success(f"🏆 **{row_b['name'].capitalize()}** gana el duelo (margen {margin:.2f}).")
    else:
    ...
    with st.expander("Ver explicación"):
        ...
        if reasons:
            ...
            for r in reasons:
                st.write("- " + r)
        st.caption("Nota: cálculo simplificado; no considera habilidades activas, objetos, clima o prioridad de movimientos.")
...
# -----------------------------
    ...
    tipo = st.selectbox("Selecciona un tipo de Pokémon:", sorted(df["type1"].dropna().unique()))
    subset = df[df["type1"] == tipo]

    col1, col2 = st.columns(2)
    with col1:
        st.bar_chart(subset[["hp","attack","defense","sp_attack","sp_defense","speed"]].mean())
    with col2:
                st.write(subset[["name","base_total"]].sort_values("base_total", ascending=False).head(10))
...
# -----------------------------
    ...
    if p_val < alpha:
        st.success("✅ *p* < 0.05 → **Se rechaza H₀**. Existe asociación significativa entre tipo y rol competitivo.")
    else:
        st.info("ℹ️ *p* ≥ 0.05 → **No se rechaza H₀**. No hay evidencia estadística de asociación.")
    ...
    st.caption("Método: Chi² de independencia por permutación — α = 0.05 | Fuente: PokeAPI + Kaggle")
    st.markdown("---")
    st.markdown("Carlos Ojea Sánchez, ***The Bridge***")
    st.markdown("Hecho usando [Streamlit](https://streamlit.io) y [PokeAPI](https://pokeapi.co).")
